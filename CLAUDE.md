...
1. **Issue first.** Every bug and every improvement gets a GitHub issue before code is touched.
   ...
   - Every issue belongs to the milestone of its stage (M0–M6); later ideas carry no milestone and the label `backlog`.
...
- `crates/` holds `server`, `model`, `vault`, `directory`, `gateway`, `browser`, `connector`, `totp` and `i18n`. `connector` depends only on `i18n` and `totp` of them: it builds for Windows too.
...
- **Configuration:** `REMOTEHUB_*` environment variables, each also as `REMOTEHUB_*_FILE` (Docker secrets); see `crates/server/src/config.rs`. Required: `REMOTEHUB_DATABASE_URL`, `REMOTEHUB_PUBLIC_URL` (the origin people open; state-changing requests from other origins are refused), `REMOTEHUB_MASTER_KEY_FILE` (only as a file; create a line with `remotehub generate-key`, the development key is `deploy/dev/master.key`). Directory: no variables; set in the setup wizard or under *Settings → Directory* (table `directory`, password sealed), used from the next sign-in on. Connecting with the own account: `REMOTEHUB_OWN_ACCOUNT_CONNECTIONS` (default `true`). Administrators: only the role in `role_assignments` (and break-glass accounts); a fresh database waits for the setup wizard, whose link `remotehub setup-code` prints. The end-to-end tests go through it first: they connect the lab's domain controller and make `RH Admins` (alice) administrators. A development database gets the same by running them once with the link (see `deploy/compose.dev.yml`, service `e2e`). RDP and VNC: `REMOTEHUB_GUACD` (`host:port`, default `guacd:4822`). SSH certificates: `REMOTEHUB_SSH_CA_KEY_FILE` (only as a file; `remotehub generate-ssh-ca`; dev and CI use the lab's `deploy/testlab/ssh/remotehub_ca`). All settings: `docs/configuration.md`.
- **CLI:** `remotehub generate-key`, `remotehub verify-audit` (exits 1 if the audit chain is broken), `remotehub break-glass create|reset|delete|list` (emergency accounts; password and TOTP secret are printed once), `remotehub recover-master-key` (reads the organisation recovery key's printed text from stdin, prints the master key file; needs only the database settings); `remotehub` alone serves. In development: `docker compose -f deploy/compose.dev.yml exec server cargo run -q -p remotehub-server -- break-glass create emergency`.
...
- **Test lab** (`deploy/testlab/`): a Samba AD domain controller `dc` (domain `REMOTEHUB.TEST`, LDAPS with the test CA in `deploy/testlab/dc/tls/ca.crt`, users and nested groups in `users.sh`), an SSH target `ssh-target`, a desktop target `desktop-target` with RDP and VNC, an OpenID Connect provider `oidc` (Dex, the provider `lab` in Kratos; see their READMEs), a mail server `mail` (Mailpit: SMTP on 1025 without TLS, the mails at `http://mail:8025/api/v1/`), and `caddy` with the ops package's Caddyfile for `remotehub.test` (its admin socket in `REMOTEHUB_TEST_CADDY_SOCKET`; it starts once the snippet it imports is written), plus `guacd` from `deploy/guacd`. All run on the compose network only; guacd is hardened as in production.
